...
| ICU-5 | provisional | **Supplied arguments retain CLDR selection while others recover, and missing arguments are withheld from the formatter.** `::testSuppliedPluralKeepsCldrSelectionWhileAnotherArgumentRecovers` — pl/ru/ar at n=3 render `few`, with en-us as the control that proves the assertion tracks CLDR rather than a string. Previously one missing argument routed the whole string to the simplified renderer, degrading every other argument's plural selection. The withholding half (`array_diff_key($params, array_flip($missing))`) is covered by the shared fixture row *"NULL is missing, not zero"*: without it, an argument missing *by being null* substitutes the recovered `{argName}` literal with an empty string and erases the gap recovery just created |
