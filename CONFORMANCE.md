...
`provisional`, never `implemented`. No SDK behaviour here reaches `implemented` until a
`contract` fixture exists that can reject a bad request and holds state across calls; CONF-2,
a rule about this record rather than about SDK behaviour, is the one exception, and its row says why.

**Grades, and what green means here.** Every rule row carries exactly one grade: `implemented`
(with a named test that fails when the behaviour is removed), `delegated` (naming the row it rests
on), `n/a` (profile or architecture, with the reason), `held (strip ruling)`, `provisional`, or
`not implemented`. Green is the first three only. **By operator ruling, `provisional` rows stay
`provisional` and name the external thing they wait on — CONF-1's `contract` fixture — rather than
being regraded upward:** re-labelling mock evidence `implemented` would contradict the paragraph
above, which is the rule this file is graded under. Each such row ends with a *Waits on* clause.
...
| GATE-3 | provisional | `::testWriteDecisionIsNeverWrittenToTheCache`, `::testWarmCacheStillResolvesPerRequestForAnIpWriteKey`, `::testResetRequestStateClearsTheWriteDecision`, and `::testWarmCacheCostsNoAuthorizationCallForAReadKey` (pins that correctness costs no per-render request) **Waits on:** CONF-1's `contract` fixture. |
| GATE-4 | provisional | `::testWriteDecisionIsNeverWrittenToTheCache` — the `authorize-project` row, where the flag sits *inside* `data` and must be stripped from the body before caching, not merely dropped with an envelope **Waits on:** CONF-1's `contract` fixture. |
| GATE-5 | provisional | `tests/Html/PageTranslatorTest.php::testDiscoveredItemsAreNotRecordedAsRegisteredWhenOnlyQueued` — a queued item is an attempt, not an acceptance, and is not recorded until the server accepts it. Also `tests/ClientTest.php::testLegacyResolvedContentBlockIsNotQueuedForRegistration` **Waits on:** CONF-1's `contract` fixture. |
...
| GATE-6 | provisional | `tests/ClientTest.php::testFlushReportsDroppedWhenTheRequestMayNotWrite` — the register half returns before the network when the request may not write. Report-half n/a: this SDK has no hint lane (HINT-2) **Waits on:** CONF-1's `contract` fixture. |

## Reading the catalog

| Rule | Status | Evidence |
|---|---|---|
| CAT-1 | provisional | `::testTranslateDoesNotQueuePhraseWithNullCatalogValue` — present-with-null is a known phrase, not a miss. `translate()` uses `array_key_exists`, not `isset` **Waits on:** CONF-1's `contract` fixture. |
| CAT-2 | provisional | `::testTranslateReturnsSourcePhraseWhenCatalogValueIsNull`, `::testTranslateInterpolatesSourcePhraseWhenCatalogValueIsNull` — the display half falls back to source text rather than rendering the null **Waits on:** CONF-1's `contract` fixture. |
| CAT-3 | provisional | `tests/ClientTest.php::testContentBlockResolvesUnderItsLegacyPipeFormId` and `::testLegacyIdResolvingToDifferentContentIsRejected` — resolution requires an array-valued entry before treating a block as known, so `custom_id: null` is never mistaken for a registered block **Waits on:** CONF-1's `contract` fixture. |

## The write lane

| Rule | Status | Evidence |
|---|---|---|
| REG-1 | provisional | `tests/ClientTest.php::testFlushReportsDroppedWhenTheRequestMayNotWrite` — the queue is dropped without a network call when the request may not write **Waits on:** CONF-1's `contract` fixture. |
...
| REG-10 | provisional | `::testFlushReportsDroppedWhenTheRequestMayNotWrite`, `::testFlushReportsSuccessWhenEverythingWasAccepted`. **Partial**: the throw-vs-swallow split across entry points is unchanged **Waits on:** CONF-1's `contract` fixture. |
...
| WIRE-1 | provisional | `tests/Http/HttpClientTest.php::testAuthenticatesWithTheXAuthorizationHeader` **Waits on:** CONF-1's `contract` fixture. |
| WIRE-2 | provisional | `tests/Http/HttpClientTest.php::testEmptyBodyOnSuccessIsNotAParseError`, `::testEmptyBodyOnErrorStatusRaisesTheMatchingException`, `::testEmptyBodyOnValidationErrorRaisesValidationException`, `::testMalformedJsonStillRaisesAParseError` **Waits on:** CONF-1's `contract` fixture. |
| WIRE-3 | provisional | `tests/ClientTest.php::testUncategorizedPhrasesDoNotSendTheSentinelOnTheWire`. All three registration paths now normalize through one `TranslatableItems::normalizeCategory()`; previously `createContentBlocks()` stripped the sentinel while `createContentBlock()` and `createPhrases()` sent it, so identical blocks landed with different stored categories. Locale form is lowercase `xx-yy`, which is correct — the backend stores and compares lowercase **Waits on:** CONF-1's `contract` fixture. |
...
Held items are not rule ids, so they do not appear in this tally; see *Held pending the operator's
strip ruling*.

```python
# Re-derive the blob and the tally together, so the header cannot drift from the body. The target
# is a COMMIT, not a branch: it is committed and deliberately unpublished, and a branch can move
# under a file that cites it.
#
...
#
# Last run 2026-09-12T23:31:34Z against blob 5c5c0723 (langsys2 @ 5cff03a1).
#
...
for m in re.finditer(r'^### ([A-Z]+-\d+)\s*—', spec, re.M):
    ...
    if pm and ('all' in pm.group(1) or 'server' in pm.group(1)):
        binding.add(m.group(1))
GRADES = ('implemented', 'delegated', 'n/a', 'held (strip ruling)', 'provisional', 'not implemented')
occ, grade, bad = collections.Counter(), {}, []
for line in open('CONFORMANCE.md').read().splitlines():
    m = re.match(r'^\|([^|]+)\|([^|]+)\|', line)
    if not m:
        continue
    cell, got = m.group(1), set()
    ...
    if not got:
        continue
    st = m.group(2).replace('*', '').strip()
    g = next((x for x in sorted(GRADES, key=len, reverse=True) if st == x or st.startswith(x + ' (')), None)
    for rid in got:
        occ[rid] += 1
        grade[rid] = g
        if g is None:
...
extra = sorted(set(occ) - set(ids))
tally = collections.Counter(grade[i] for i in ids if i in grade)
green = all(grade.get(i) in ('implemented', 'delegated', 'n/a') for i in ids)
print(f"rule ids {len(ids)}  once {sum(occ[i] == 1 for i in ids)}  missing {missing}  duplicated {dup}  unknown {extra}")
print(f"binding (all | server) {len(binding)}")
print('grades', {g: tally.get(g, 0) for g in GRADES})
print('unrecognised grades', bad)
print('GREEN', 'yes' if green else 'no')
sys.exit(1 if (missing or dup or extra or bad) else 0)
# EOF
```
