...
| ICU-1 | partial | n/a (pure) | **Mutation, this regrade** (full suite per mutant, file restored byte-for-byte after each, on the `src/` of `5400248`): in the interpolator, treating no argument as missing reddens 17 including `::testIcuRecoversWhenTheCallerSuppliesNoParamsAtAll`, `::testMissingSelectArgumentRendersTheNeutralBranch`. **Client route, pinned since:** re-adding the empty-params short-circuit to `Client::interpolate()` reddens only `tests/Format/ClientInterpolationTest.php::testInterpolationFixtureHoldsThroughTranslate`, on the row *select: no params at all falls to other*. That test drives the shared interpolation fixture through `translate()`, and the fixture now carries a select and a plural called with no params and with an empty map; the same mutant against the previous 19-row fixture stays green, so the new rows are what catch it (mutants run in a worktree). **Page route defect, measured:** `translatePage()` with no params renders a catalog value that holds ICU as raw MessageFormat source, `<p>{name_gender, select, male {Bienvenido} female {Bienvenida} other {Bienvenide}}</p>`, while `translate()` and `translateContentBlock()` recover the same value to `Bienvenide`. The cause is the empty-params early return in `PageTranslator::interp()`: removing it renders `<p>Bienvenide</p>`. Behaviour unchanged pending the operator's word. *Earlier record:* A missing `select`/`plural` argument selects the `other` branch. `tests/Format/InterpolatorTest.php::testIcuRecoversWhenTheCallerSuppliesNoParamsAtAll` covers the case that previously escaped — the empty-params short-circuit in `Client::interpolate()` is gone, so a phrase whose ICU the caller knows nothing about still recovers |
| ICU-2 | implemented | n/a (pure) | **Mutation, this regrade** (full suite per mutant, file restored byte-for-byte after each, on the `src/` of `5400248`): counting a present-but-null argument as supplied reddens `::testInterpolationMatchesTheReferenceFixtures`, `::testRecoveredLiteralSurvivesTheFormatterCall`. Every public route reaches `Interpolator::interpolate()` whenever the caller supplies params. The empty-params bypasses, the Client's former short-circuit (now pinned) and the page path's (measured live), are ICU-1's finding: they stop recovery from starting at all, while this rule governs recovery once it runs. *Earlier record:* A present-but-null argument is treated as absent. Covered by the shared fixture row *"NULL is missing, not zero"* in `tests/fixtures/interpolation-reference.json`, asserted by `::testInterpolationMatchesTheReferenceFixtures`. Recovery withholds missing arguments from intl so a null cannot substitute the recovered `{argName}` with empty |
| ICU-3 | implemented | n/a (pure) | **Mutation, this regrade** (full suite per mutant, file restored byte-for-byte after each, on the `src/` of `5400248`): recovery not descending into a supplied node reddens `::testRecoveryDescendsIntoASuppliedNode`, `::testInterpolationMatchesTheReferenceFixtures`; a recovered `#` rendering `0` instead of `{argName}` reddens `::testMissingPluralArgumentKeepsTheSentenceAndShowsTheGap`, `::testIcuRecoversWhenTheCallerSuppliesNoParamsAtAll`, `::testRecoveredLiteralSurvivesTheFormatterCall`, `::testRecoveryDescendsIntoASuppliedNode`, `::testInterpolationMatchesTheReferenceFixtures`. Every public route reaches `Interpolator::interpolate()` whenever the caller supplies params. The empty-params bypasses, the Client's former short-circuit (now pinned) and the page path's (measured live), are ICU-1's finding: they stop recovery from starting at all, while this rule governs recovery once it runs. *Earlier record:* `#` renders the literal `{argName}` and recovery descends into nested nodes: `::testRecoveryDescendsIntoASuppliedNode`. That test is **characterisation, not a regression pin** — a missing argument has no CLDR category to lose, so it produces the same string under the previous implementation; the discriminating vectors are in `cldrFewProvider` |
| ICU-4 | implemented | n/a (pure) | **Mutation, this regrade** (full suite per mutant, file restored byte-for-byte after each, on the `src/` of `5400248`): never logging the defaulted-argument notice reddens `::testRecoveryEmitsADebugNoticeNamingEveryDefaultedArgument`, `::testRecoveryNoticeFiresForPluralAndNumberArgumentsToo`, `::testRecoveryNoticeIsDedupedPerLocaleAndTemplate`. Every public route reaches `Interpolator::interpolate()` whenever the caller supplies params. The empty-params bypasses, the Client's former short-circuit (now pinned) and the page path's (measured live), are ICU-1's finding: they stop recovery from starting at all, while this rule governs recovery once it runs. *Earlier record:* `::testRecoveryEmitsADebugNoticeNamingEveryDefaultedArgument` asserts the notice fires **and** names every defaulted argument and the locale — the rule's own test requirement, since a test that only checks the rendered string passes whether or not anyone can diagnose it. `::testRecoveryNoticeIsDedupedPerLocaleAndTemplate` pins the dedup, `::testNoNoticeWhenNothingWasDefaulted` the negative |
| ICU-5 | implemented | n/a (pure) | **Mutation, this regrade** (full suite per mutant, file restored byte-for-byte after each, on the `src/` of `5400248`): handing intl the missing arguments too reddens `::testRecoveredLiteralSurvivesTheFormatterCall`, `::testInterpolationMatchesTheReferenceFixtures`. Every public route reaches `Interpolator::interpolate()` whenever the caller supplies params. The empty-params bypasses, the Client's former short-circuit (now pinned) and the page path's (measured live), are ICU-1's finding: they stop recovery from starting at all, while this rule governs recovery once it runs. *Earlier record:* **Outcome:** a recovered `{argName}` literal survives the formatter call, and a supplied `select`/`plural` keeps full CLDR selection while other arguments recover. `::testSuppliedPluralKeepsCldrSelectionWhileAnotherArgumentRecovers` — pl/ru/ar at n=3 render `few`, with en-us as the control proving the assertion tracks CLDR rather than a string. `::testRecoveredLiteralSurvivesTheFormatterCall` covers the literal's survival across typed arguments, absent **and** present-and-null, with supplied controls. **Mechanism here is `array_diff_key($params, array_flip($missing))`** — one conforming way to reach the outcome, not the outcome itself: the JS leg conforms differently, by replacing the node so no binding site exists for the formatter to fill. Confirmed against the final clause (langsys `3712d5a2`), which states the obligation as the outcome and names both mechanisms as conforming |
...
| WIRE-3 | partial | mock | **Category half:** `tests/ClientTest.php::testUncategorizedPhrasesDoNotSendTheSentinelOnTheWire`. All three registration paths now normalize through one `TranslatableItems::normalizeCategory()`; previously `createContentBlocks()` stripped the sentinel while `createContentBlock()` and `createPhrases()` sent it, so identical blocks landed with different stored categories. **Locale half, measured (Laravel lane's finding, reproduced):** only `setLocale()` normalises. After `setLocale('es-es')` and `translate('Save')`, a call to `translate('Save', 'es-ES')` fetched the catalog a second time and sent `locale: es-ES` on the wire, and the instance's memory catalog held both `es-es` and `es-ES`; `getTranslations()` takes the locale as given in the same way. `FileCache` did not split on this machine, most likely because the macOS filesystem is case-insensitive: `FileCache` sanitises keys but does not lowercase them, so a case-sensitive filesystem or Redis would split — that part is inferred, not measured. The row previously said the locale form was correct; it is correct only through `setLocale()`. **Waits on:** locale normalisation on every public entry point that accepts one (an in-lane build, on the operator's word), and CONF-1's `contract` fixture for the category half. |
