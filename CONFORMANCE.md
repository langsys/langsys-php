...
| REG-8 | not implemented | - | A failed send retains the queue but there is no retry and no backoff. **Measured for long-lived workers (Laravel lane's finding, reproduced):** the retained queue survives `resetRequestState()`, so in a queue worker or under Octane a failing endpoint receives a larger POST on every request — four requests against a failing endpoint sent 1, 2, 3 and 4 items, with no backoff between them. That is the load escalation REG-8's *Why* describes. **Held** for the per-request server clarification now with the operator: under FPM, backoff state has to outlive the request and be keyed per project, and anything shared at process or host level reproduces CACHE-1's problem for backoff. |
...
#
# Last run 2026-09-23T17:22:24Z against blob e2524fd6 (langsys2 @ 68d9da48): exit 0.
#
...
### Held pending the operator's strip ruling

Graded `held (strip ruling)` and deliberately not built. If C0 controls are stripped, the characters
are REMOVED rather than kept, and each of these changes shape.

| Item | Status | What it is |
|---|---|---|
| C0 parser split | held (strip ruling) | libxml2 2.9.13 drops 28 C0 controls from DOM text — U+0001–0008, U+000B, U+000C, U+000E–001F — including as character references (`a&#x1C;b&#x0B;c` yields `abc`), while keeping TAB, LF and CR. parse5 7.3.0 and happy-dom 20.10.2 keep all 28. Measured here on the raw `DOMDocument`, with no SDK code in the path |
| U+001C–U+001F non-membership test | held (strip ruling) | To be asserted on `Whitespace::collapse` directly, never through a DOM path, where libxml2 has already deleted the character and the test would pass for that reason. If the strip is adopted the expectation flips from kept to removed |
| VT/FF change | held (strip ruling) | No change to U+000B/U+000C handling until the ruling |
